<script lang="ts">
  ...
  $: dep = deploymentDetails || deployment;
  ...
  function getDeploymentStatus(d: any): string {
    if (!d) return 'Unknown';
    const availableCond = d.status?.conditions?.find((c: any) => c.type === 'Available');
    const progressing = d.status?.conditions?.find((c: any) => c.type === 'Progressing');
    const des = d.spec?.replicas || 0;
    const upd = d.status?.updatedReplicas || 0;
    const rdy = d.status?.readyReplicas || 0;
    const cur = d.status?.replicas || 0;
    
    const isRollingOut = (upd < des) || 
                        (progressing?.status === 'True' && rdy < des && upd > 0);
    const isScaling = cur !== des && !isRollingOut;
    
    if (isRollingOut) return 'Restarting';
    if (isScaling) return 'Scaling';
    if (availableCond?.status === 'True' && rdy === des) return 'Ready';
    if (rdy < des) return 'Progressing';
    return 'Unknown';
  }
  ...
  async function loadManagedReplicaSets() {
    if (!deployment?.metadata?.name || !deployment?.metadata?.namespace) return;
    replicasetsLoading = true; replicasetsError = null;
    try {
      const replicasets = await invoke('kuboard_get_deployment_replicasets', {
        name: deployment.metadata.name,
        namespace: deployment.metadata.namespace
      }) as any[];
      managedReplicaSets = replicasets || [];
    } catch (err: any) {
      console.warn('Failed to load managed replicasets:', err);
      managedReplicaSets = [];
    } finally {
  ...
  async function restartDeployment() {
    if (!deployment?.metadata?.name || !deployment?.metadata?.namespace) return;
    restartLoading = true; restartError = null;
    try {
...
<div class="resource-details-view">
  ...
  <div class="details-nav-bar">
    <div class="nav-actions">
      <button class="btn-back" onclick={() => { if (onBack) onBack(); dispatch('back'); }}>← Back to Deployments</button>
  ...
  <div class="details-sheet">
    ...
    <div class="sheet-section specs-strip">
      ...
      <div class="spec-cell">
        <span class="spec-label">Updated Replicas</span>
        <span class="spec-val">{updated}</span>
      </div>
      <div class="spec-cell">
        <span class="spec-label">Strategy</span>
        <span class="spec-val">{dep?.spec?.strategy?.type || 'RollingUpdate'}</span>
      </div>
      {#if owner}
        <div class="spec-cell">
      ...
      <div class="spec-cell">
        ...
        <span class="spec-val">{formatAge(dep?.metadata?.creationTimestamp)}</span>
      </div>
    </div>
    ...
    {#if showScaleInput}
      <div class="sheet-section scale-inline-box">
        ...
        <button class="btn-primary-sm" onclick={scaleDeployment} disabled={scaleLoading}>
          {scaleLoading ? 'Scaling...' : 'Apply Scale'}
        </button>
        {#if scaleError}
          <span class="err-txt">⚠️ {scaleError}</span>
        {/if}
      </div>
    {/if}
    ...
    <div class="sheet-section">
      <h5>🔄 ReplicaSets ({managedReplicaSets.length})</h5>
      {#if replicasetsLoading}
        <div class="muted-text">⏳ Loading ReplicaSets...</div>
      ...
      {:else}
        <p class="muted-text">No active ReplicaSets found</p>
      {/if}
    </div>
    ...
    <div class="sheet-section">
      ...
      {:else}
        <p class="muted-text">No active pods managed by this Deployment</p>
      {/if}
    </div>
...
{#if yamlViewerVisible}
  <div class="modal-overlay" onclick={closeYamlViewer} role="button" tabindex="-1" onkeydown={(e) => e.key === 'Escape' && closeYamlViewer()}>
    <div class="modal-box" onclick={(e) => e.stopPropagation()} role="dialog" tabindex="-1">
      <div class="modal-hdr">
        <h4>Deployment YAML: {deployment?.metadata?.name}</h4>
...
<style>
  ...
  .rs-table, .pods-table { border: 1px solid var(--border-primary); border-radius: var(--radius-sm); overflow: hidden; margin-bottom: 8px; }
  .rs-head { display: grid; grid-template-columns: 1fr 180px 100px; background: rgba(255, 255, 255, 0.03); padding: 8px 12px; font-size: 0.8rem; font-weight: 600; color: var(--text-secondary); border-bottom: 1px solid var(--border-primary); }
  .rs-row { display: grid; grid-template-columns: 1fr 180px 100px; padding: 8px 12px; font-size: 0.85rem; border-bottom: 1px solid rgba(255, 255, 255, 0.04); align-items: center; }
