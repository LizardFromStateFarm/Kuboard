...
<script lang="ts">
  ...
  function startAutoRefresh() {
    ...
    refreshInterval = setInterval(() => {
      ...
      if (lastRestartTime && (now - lastRestartTime) < 30000) {
        console.log('Auto-refreshing statefulsets after restart');
        dispatch('reload');
      } else {
  ...
  function handleViewYaml(event: any) {
    console.log('handleViewYaml called in StatefulSetsPanel', event);
  ...
  function handleActionEdit(event: any) {
    console.log('handleActionEdit called in StatefulSetsPanel', event);
  ...
  async function saveYaml() {
    if (!contextMenuStatefulSet?.metadata?.name || !contextMenuStatefulSet?.metadata?.namespace) return;
    ...
    try {
      ...
      await invoke('kuboard_update_statefulset', {
        name: contextMenuStatefulSet.metadata.name,
        namespace: contextMenuStatefulSet.metadata.namespace,
        statefulset: parsed
      });
      closeYamlEditor();
    } catch (err: any) {
    ...
    <div class="yaml-viewer-modal" onclick={closeYamlViewer}>
      <div class="yaml-viewer-content" onclick={(e) => e.stopPropagation()}>
        <div class="yaml-viewer-header">
          <h3>StatefulSet YAML: {contextMenuStatefulSet?.metadata?.name}</h3>
...
<style>
  ...
  /* YAML Viewer/Editor Styles - same as DeploymentsPanel */
  .yaml-viewer-modal {
    position: fixed;
    top: 0;
    left: 0;
