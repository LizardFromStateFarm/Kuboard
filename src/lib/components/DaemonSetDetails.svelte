<script lang="ts">
  ...
  import { ArrowLeft, FileText, Settings, ExternalLink, Box, Sliders, Tag, Loader2, Trash2 } from 'lucide-svelte';
  ...
  async function handleActionDelete() {
    if (!ds?.metadata?.name || !ds?.metadata?.namespace) return;
    if (!confirm(`Are you sure you want to delete DaemonSet "${ds.metadata.name}"?`)) return;
    try {
...
<div class="resource-details-view">
  ...
  <div class="details-nav-bar">
    <div class="nav-actions">
      ...
      <button class="btn-subtle danger" onclick={handleActionDelete} title="Delete DaemonSet"><Trash2 size={14} class="inline-icon" /> Delete</button>
    </div>
    <div class="nav-heading">
      <span class="status-pill status-{getStatusClass(status)}">{status}</span>
      <h3 class="nav-title clickable" onclick={() => copyToClipboard(ds?.metadata?.name, 'DaemonSet Name')} title="Click to copy daemonset name">
        {ds?.metadata?.name} {#if copiedNotice}<span class="copy-toast-inline">{copiedText}</span>{/if}
      </h3>
...
{#if yamlEditorVisible}
  <YamlEditor
    resource={ds}
    resourceType="daemonset"
    onSave={() => { yamlEditorVisible = false; loadDaemonSetDetails(); }}
