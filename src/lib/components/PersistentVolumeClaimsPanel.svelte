<!-- Kuboard PersistentVolumeClaims Panel Component -->
<script lang="ts">
  ...
  async function fetchPVCs() {
    ...
    try {
      loading = true;
      pvcs = await invoke('kuboard_list_persistent_volume_claims', { namespace });
      error = null;
    } catch (e: any) {
      console.error('Failed to fetch PVCs:', e);
      error = String(e);
    } finally {
      loading = false;
    }
  ...
  onMount(() => {
    fetchPVCs();
    refreshTimer = setInterval(fetchPVCs, 10000); // Refresh every 10s
  });
...
<div class="pvc-panel">
  ...
  {#if loading && pvcs.length === 0}
    <div class="loading-state">
      ...
      <p>Loading Persistent Volume Claims...</p>
    </div>
  {:else if error}
    <div class="error-state">
      <span class="error-icon">⚠️</span>
      <p>{error}</p>
      <button onclick={fetchPVCs}>Retry</button>
    </div>
  {:else}
    <ResourceTable
      items={pvcs}
      filteredItems={filteredPVCs}
      bind:searchQuery
      searchPlaceholder="Search PVCs..."
      noItemsMessage="No Persistent Volume Claims found."
      noSearchResultsMessage="No PVCs match your search query:"
    >
      ...
      <svelte:fragment slot="rows">
        {#each filteredPVCs as pvc (pvc.metadata.uid)}
          <tr class="resource-row" oncontextmenu={(e) => handleContextMenu(e, pvc)}>
            <td class="name-cell">{pvc.metadata.name}</td>
            <td>
              <span class="status-badge status-{getStatusClass(pvc.status?.phase)}">
            ...
            <td>{formatAge(pvc.metadata.creationTimestamp)}</td>
            <td class="actions-cell">
              <button class="action-btn" onclick={(e) => handleContextMenu(e, pvc)}>⚙️</button>
            </td>
          </tr>
        {/each}
      </svelte:fragment>
