<script lang="ts">
  ...
  export let statefulSet: any;
  export let onBack: () => void;

  let statefulSetDetails: any = null;
  let managedPods: any[] = [];
  let loading = false;
  ...
  function getPodOrdinal(podName: string, ssName: string): number {
    // Extract ordinal from pod name (e.g., "web-0" from "web")
    const match = podName.match(new RegExp(`${ssName}-(\\d+)$`));
  ...
  async function loadStatefulSetDetails() {
    if (!statefulSet?.metadata?.name || !statefulSet?.metadata?.namespace) return;
    
    loading = true;
    error = null;
    
    try {
      const ssData = await invoke('kuboard_get_statefulset', {
        name: statefulSet.metadata.name,
        namespace: statefulSet.metadata.namespace
      }).catch(() => null);
      
      statefulSetDetails = ssData || statefulSet;
      scaleValue = statefulSetDetails.spec?.replicas || 0;
      
      // Load managed pods
      await loadManagedPods();
    } catch (err) {
      error = String(err);
      statefulSetDetails = statefulSet; // Fallback to passed statefulSet
      scaleValue = statefulSet.spec?.replicas || 0;
    } finally {
      loading = false;
    }
  }
  ...
  async function loadManagedPods() {
    ...
    try {
      const pods = await invoke('kuboard_get_statefulset_pods', {
        name: statefulSet.metadata.name,
        namespace: statefulSet.metadata.namespace
      });
  ...
  async function scaleStatefulSet() {
    ...
    try {
      await invoke('kuboard_scale_statefulset', {
        name: statefulSet.metadata.name,
        namespace: statefulSet.metadata.namespace,
        replicas: scaleValue
      });
  ...
  function handleAction(event: CustomEvent) {
    ...
    switch (action) {
      ...
      case 'edit':
        ...
        break;
      case 'copy-name':
        copyToClipboard(statefulSet?.metadata?.name || '');
        break;
      case 'copy-namespace':
        copyToClipboard(statefulSet?.metadata?.namespace || '');
        break;
      default:
  ...
  async function openYamlViewer() {
    ...
    try {
      yamlContent = JSON.stringify(statefulSetDetails || statefulSet, null, 2);
  ...
  function openYamlEditor() {
    yamlEditorContent = JSON.stringify(statefulSetDetails || statefulSet, null, 2);
  ...
  async function saveYaml() {
    ...
    try {
      // TODO: Implement statefulset update command
      alert('StatefulSet update not yet implemented. Please use kubectl.');
      closeYamlEditor();
    } catch (error: any) {
...
<div class="full-details-view">
  <div class="details-header">
    <div class="header-left">
      <button class="back-button" onclick={onBack}>← Back to StatefulSets</button>
      <button class="actions-button" onclick={openActionsMenu}>⚙️ Actions</button>
    </div>
    <div class="header-right">
      <h3>{statefulSet?.metadata?.name}</h3>
      <span class="statefulset-namespace">({statefulSet?.metadata?.namespace})</span>
    </div>
  </div>
  ...
  {:else if error}
    <div class="error-message">
      ...
      <div class="error-content">
        <h5>Failed to load StatefulSet details</h5>
        <p>{error}</p>
        <button class="retry-button" onclick={loadStatefulSetDetails}>Retry</button>
      </div>
    </div>
  {:else}
    {@const ss = statefulSetDetails || statefulSet}
    {@const status = getStatefulSetStatus(ss)}
    {@const desired = ss.spec?.replicas || 0}
    {@const ready = ss.status?.readyReplicas || 0}
    {@const current = ss.status?.currentReplicas || 0}
    {@const updated = ss.status?.updatedReplicas || 0}
    {@const strategy = getUpdateStrategy(ss)}
    {@const policy = getPodManagementPolicy(ss)}
    
    <div class="statefulset-details-content">
      <div class="details-section">
        ...
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <div class="info-value-container">
              <span class="info-value" title={ss.metadata?.creationTimestamp}>{formatAge(ss.metadata?.creationTimestamp)}</span>
              <button class="copy-button" onclick={() => copyToClipboard(ss.metadata?.creationTimestamp || '')}>📋</button>
            </div>
          </div>
        </div>
      </div>
        ...
        {#if showScaleInput}
          <div class="scale-controls">
            <div class="scale-input-group">
              ...
              />
              <button class="scale-button" onclick={scaleStatefulSet} disabled={scaleLoading}>
      ...
      <div class="details-section">
        <h6>StatefulSet Actions</h6>
        <div class="action-buttons">
          <button class="restart-button" onclick={restartStatefulSet} disabled={restartLoading}>
          ...
          <div class="info-grid">
            ...
            {#if strategy === 'RollingUpdate'}
              <div class="info-item">
                ...
                <div class="info-value-container">
                  <span class="info-value">{ss.spec?.updateStrategy?.rollingUpdate?.partition?.toString() || 'Not set'}</span>
                </div>
              </div>
            {/if}
          </div>
      ...
      <div class="details-section">
        ...
        {#if !sectionsCollapsed.conditions}
          {#if ss.status?.conditions && ss.status.conditions.length > 0}
            <div class="conditions-table">
              ...
              {#each ss.status.conditions as condition}
      ...
      <div class="details-section">
        ...
        {#if !sectionsCollapsed.pods}
              ...
              {#each managedPods as pod}
                <div class="pod-row">
                  <div class="pod-name">
                    ...
                    <span class="pod-ordinal">#{getPodOrdinal(pod.metadata?.name || '', ss.metadata?.name || '')}</span>
                  </div>
          ...
          {:else}
            <div class="pods-placeholder"><p>No pods managed by this StatefulSet</p></div>
          {/if}
        {/if}
      </div>
      ...
      <div class="details-section">
        <h6 class="section-header" onclick={() => toggleSection('volumes')} role="button" tabindex="0" onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('volumes')}>
          <span>Volume Claim Templates</span>
          <span class="collapse-icon">{sectionsCollapsed.volumes ? '▶' : '▼'}</span>
        </h6>
        {#if !sectionsCollapsed.volumes}
          {#if ss.spec?.volumeClaimTemplates && ss.spec.volumeClaimTemplates.length > 0}
            <div class="volumes-section">
              {#each ss.spec.volumeClaimTemplates as vct}
                <div class="volume-claim-item">
                  <div class="volume-claim-name">{vct.metadata?.name || 'Unnamed'}</div>
                  <div class="volume-claim-info">
                    Storage: {vct.spec?.resources?.requests?.storage || 'Not specified'}
                    {#if vct.spec?.storageClassName}
                      <br>Storage Class: {vct.spec.storageClassName}
                    {/if}
                    {#if vct.spec?.accessModes && vct.spec.accessModes.length > 0}
                      <br>Access Modes: {vct.spec.accessModes.join(', ')}
                    {/if}
                  </div>
                </div>
              {/each}
          ...
          {:else}
            <div class="volumes-placeholder"><p>No volume claim templates defined</p></div>
          {/if}
        {/if}
      </div>
        ...
        {#if !sectionsCollapsed.selectors}
          <div class="info-grid">
            <div class="info-item">
              ...
              <div class="info-value-container">
                {#if ss.spec?.selector?.matchLabels && Object.keys(ss.spec.selector.matchLabels).length > 0}
                  <div class="kv-list">
                    {#each Object.entries(ss.spec.selector.matchLabels) as [k, v]}
            ...
            <div class="info-item">
              ...
              <div class="info-value-container">
                {#if ss.spec?.selector?.matchExpressions && ss.spec.selector.matchExpressions.length > 0}
                  <div class="match-expressions-list">
                    {#each ss.spec.selector.matchExpressions as expr}
        ...
        {#if !sectionsCollapsed.labels}
          <div class="info-grid">
            <div class="info-item">
              ...
              <div class="info-value-container">
                <div class="kv-list">
                  {#if ss.metadata?.labels && Object.keys(ss.metadata.labels).length > 0}
                    {#each Object.entries(ss.metadata.labels) as [k, v]}
                      <div class="kv"><span class="k">{k}</span><span class="v">{v}</span></div>
                    {/each}
                  {:else}
                    <span class="info-value">-</span>
            ...
            <div class="info-item">
              ...
              <div class="info-value-container">
                <div class="kv-list">
                  {#if ss.metadata?.annotations && Object.keys(ss.metadata.annotations).length > 0}
                    {#each Object.entries(ss.metadata.annotations) as [k, v]}
                      <div class="kv"><span class="k">{k}</span><span class="v">{v}</span></div>
                    {/each}
                  {:else}
                    <span class="info-value">-</span>
  ...
  <div class="yaml-viewer-modal" onclick={(e) => e.target === e.currentTarget && closeYamlViewer()}>
    <div class="yaml-viewer-content" onclick={(e) => e.stopPropagation()}>
      <div class="yaml-viewer-header">
        <h3>StatefulSet YAML: {statefulSet?.metadata?.name}</h3>
...
{#if yamlEditorVisible}
  <div class="yaml-viewer-modal" onclick={(e) => e.target === e.currentTarget && closeYamlEditor()}>
    <div class="yaml-viewer-content yaml-editor-content" onclick={(e) => e.stopPropagation()}>
      <div class="yaml-viewer-header">
        <h3>Edit StatefulSet YAML: {statefulSet?.metadata?.name}</h3>
