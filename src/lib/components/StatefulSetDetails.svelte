<script lang="ts">
  ...
  import { ArrowLeft, Sliders, FileText, Settings, ExternalLink, AlertTriangle, Box, Tag, Loader2, Trash2, RefreshCw } from 'lucide-svelte';
  ...
  $: sts = statefulSetDetails || statefulSet;
  $: status = getStatefulSetStatus(sts);
  $: desired = sts?.spec?.replicas || 0;
  $: ready = sts?.status?.readyReplicas || 0;
  $: current = sts?.status?.replicas || 0;
  $: owner = getOwnerReference(sts);

  function getOwnerReference(s: any): { type: string; name: string } | null {
    const ownerRefs = s?.metadata?.ownerReferences || [];
  ...
  function getStatefulSetStatus(s: any): string {
    if (!s) return 'Unknown';
    const des = s.spec?.replicas || 0;
    const rdy = s.status?.readyReplicas || 0;
    const cur = s.status?.replicas || 0;
  ...
  async function loadStatefulSetDetails() {
    ...
    try {
      const details = await invoke('kuboard_get_statefulset_details', {
        name: statefulSet.metadata.name,
        namespace: statefulSet.metadata.namespace
      });
      statefulSetDetails = details;
      scaleValue = statefulSetDetails?.spec?.replicas || statefulSet?.spec?.replicas || 0;
      await loadManagedPods();
    } catch (err: any) {
      console.warn('Failed to load statefulset details via Tauri API:', err);
  ...
  async function handleActionDelete() {
    if (!sts?.metadata?.name || !sts?.metadata?.namespace) return;
    if (!confirm(`Are you sure you want to delete StatefulSet "${sts.metadata.name}"?`)) return;
    try {
  ...
  onMount(() => { loadStatefulSetDetails(); });
</script>
...
<div class="resource-details-view">
  ...
  <div class="details-nav-bar">
    <div class="nav-actions">
      <button class="btn-back" onclick={() => { if (onBack) onBack(); dispatch('back'); }}><ArrowLeft size={14} class="inline-icon" /> Back to StatefulSets</button>
      ...
      <button class="btn-subtle danger" onclick={handleActionDelete} title="Delete StatefulSet"><Trash2 size={14} class="inline-icon" /> Delete</button>
    </div>
    <div class="nav-heading">
      <span class="status-pill status-{getStatusClass(status)}">{status}</span>
      <h3 class="nav-title clickable" onclick={() => copyToClipboard(sts?.metadata?.name, 'StatefulSet Name')} title="Click to copy statefulset name">
        {sts?.metadata?.name} {#if copiedNotice}<span class="copy-toast-inline">{copiedText}</span>{/if}
      </h3>
      <span class="namespace-pill">{sts?.metadata?.namespace}</span>
    </div>
  </div>
  ...
  <div class="details-sheet">
    ...
    <div class="sheet-section specs-strip">
      ...
      <div class="spec-cell">
        <span class="spec-label">Current Replicas</span>
        <span class="spec-val">{current}</span>
      </div>
      <div class="spec-cell">
        <span class="spec-label">Service Name</span>
        <span class="spec-val">{sts?.spec?.serviceName || '-'}</span>
      </div>
      {#if owner}
        <div class="spec-cell">
      ...
      <div class="spec-cell">
        ...
        <span class="spec-val">{formatAge(sts?.metadata?.creationTimestamp)}</span>
      </div>
    </div>
    ...
    {#if showScaleInput}
      <div class="sheet-section scale-inline-box">
        ...
        <button class="btn-primary-sm" onclick={scaleStatefulSet} disabled={scaleLoading}>
    ...
    <div class="sheet-section">
      ...
      {:else}
        <p class="muted-text">No active pods managed by this StatefulSet</p>
      {/if}
    </div>
    ...
    <div class="sheet-section">
      ...
      <div class="kv-grid">
        <div class="kv-block">
          ...
          <div class="tag-wrap">
            {#if sts?.spec?.selector?.matchLabels && Object.keys(sts.spec.selector.matchLabels).length > 0}
              {#each Object.entries(sts.spec.selector.matchLabels) as [k, v]}
    ...
    <div class="sheet-section">
      ...
      <div class="kv-grid">
        <div class="kv-block">
          ...
          <div class="tag-wrap">
            {#if sts?.metadata?.labels && Object.keys(sts.metadata.labels).length > 0}
              {#each Object.entries(sts.metadata.labels) as [k, v]}
                <span class="flat-tag"><strong class="k">{k}:</strong> {v}</span>
              {/each}
            {:else}
              <span class="muted-text">-</span>
        ...
        <div class="kv-block">
          ...
          <div class="tag-wrap">
            {#if sts?.metadata?.annotations && Object.keys(sts.metadata.annotations).length > 0}
              {#each Object.entries(sts.metadata.annotations) as [k, v]}
...
{#if yamlEditorVisible}
  <YamlEditor
    resource={sts}
    resourceType="statefulset"
    onSave={() => { yamlEditorVisible = false; loadStatefulSetDetails(); }}
...
{#if yamlViewerVisible}
  <div class="modal-overlay" onclick={closeYamlViewer} role="button" tabindex="-1" onkeydown={(e) => e.key === 'Escape' && closeYamlViewer()}>
    <div class="modal-box" onclick={(e) => e.stopPropagation()} role="dialog" tabindex="-1">
      <div class="modal-hdr">
        <h4>StatefulSet YAML: {statefulSet?.metadata?.name}</h4>
