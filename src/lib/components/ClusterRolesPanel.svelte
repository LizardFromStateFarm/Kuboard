<!-- Kuboard ClusterRoles Panel Component -->
<script lang="ts">
  ...
  async function fetchClusterRoles() {
    ...
    try {
      loading = true;
      clusterRoles = await invoke('kuboard_list_cluster_roles');
      error = null;
    } catch (e: any) {
      console.error('Failed to fetch ClusterRoles:', e);
      error = String(e);
    } finally {
      loading = false;
    }
  ...
  onMount(() => {
    fetchClusterRoles();
    refreshTimer = setInterval(fetchClusterRoles, 30000); // Refresh every 30s
  });
...
<div class="cluster-roles-panel">
  ...
  {:else if error}
    <div class="error-state">
      ...
      <p>{error}</p>
      <button onclick={fetchClusterRoles}>Retry</button>
    </div>
  {:else}
    <ResourceTable
      items={clusterRoles}
      ...
      noItemsMessage="No Cluster Roles found."
      noSearchResultsMessage="No Cluster Roles match your search query:"
    >
